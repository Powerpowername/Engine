cbuffer SceneConstants : register(b0)
{
    ...
    float4 gCameraPos;
    // 每层 clipmap 在全局 level 0 网格坐标中的最小坐标起点。
    ...
    float4 gRenderTerrainSampleParams;//最大最小暂时是没又用的啊，有用的目前看来值用上z这个坐标，w也没用上，主要是为了对齐
...
Texture2DArray gCascadeShadowMap : register(t1);
...
struct VSInput
{
    //Grid 就是static_mesh.vertices网格数据
...
VSOutput VSMain(VSInput input)
{
    ...
    uint levelStep = 1u << output.level;//当前 clipmap 层的网格放大倍率
...
uint GetCascadeIndex(float3 worldPos, float3 cameraPos)
{
...
float SampleCascadeShadow(float3 worldPos, float3 cameraPos)
{
    uint cascadeIndex = GetCascadeIndex(worldPos, cameraPos);
    float4 lightClip = mul(float4(worldPos, 1.0), gLightViewProj[cascadeIndex]);
    float invW = rcp(max(lightClip.w, 0.0001));
...
{
    float3 lightDir = normalize(-gLightDirAndCount.xyz);
    float diffuse = saturate(dot(lightDir, input.normal));
    float shadow = SampleCascadeShadow(input.worldPos, gCameraPos.xyz);
    float lighting = 0.25 + diffuse * lerp(0.35, 1.0, shadow) * 0.75;
    float3 albedo = lerp(float3(0.10, 0.50, 0.18), float3(0.72, 0.72, 0.69), saturate(diffuse));
    float3 baseColor = albedo * lighting;
...
cbuffer ComputeConstants : register(b0)
{
    ...
    uint4 gComputeClipmapUpdateParams;
    // xy=地形世界尺寸，zw=地形中心偏移。
    float4 gComputeTerrainParams;
    // x=clipmap_scale，即 level 0 一个网格单位对应的世界距离。
    float4 gComputeScaleParams;
...
float2 WorldToSourceUv(float2 worldXZ)
{
    ...
    return (worldXZ - gComputeTerrainParams.zw) / gComputeTerrainParams.xy + float2(0.5, 0.5);
}
...
float3 GetTerrain(float2 worldXZ)
{
    ...
    float safeResolution = (float)max((int)gComputeClipmapUpdateParams.z - 1, 1);
    float2 texelWorldSize = gComputeTerrainParams.xy / float2(safeResolution, safeResolution);
...
void CSMain(uint3 dispatchThreadId : SV_DispatchThreadID)
{
    ...
    if (updateIndex >= gComputeClipmapUpdateParams.w) {
        return;
    }
    ...
    float2 worldXZ = gComputeScaleParams.x * float2(levelTexel * levelStep);
